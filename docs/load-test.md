...
현재 좌석 조회 API는 선택 구역의 정적 좌석 2,500건을 DB에서 읽고, 같은 수의 좌석 ID로
Redis `MGET`을 수행해 임시 선점 상태를 합친 뒤 JSON 응답을 만듭니다. 따라서 좌석 조회의
응답 시간은 DB 조회뿐 아니라 Redis 조회량, DTO 생성, JSON 직렬화, 네트워크 응답 크기의
영향을 함께 받습니다.
...
| `seat.query.hold.read` | 좌석 ID 목록에 대한 Redis hold 상태 `MGET` 시간 |
