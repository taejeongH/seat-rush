...
## 개선 4: 구역별 hold 인덱스 적용

### 문제

기존 좌석 조회는 구역의 2,500개 좌석 ID마다 hold key를 생성하고 Redis `MGET`으로 선점 상태를 확인했습니다.
실제 선점 좌석 수와 관계없이 같은 수의 key와 응답 값을 처리해야 했습니다.

### 적용한 변경

- 좌석별 TTL hold key는 유지합니다.
- 구역별 Sorted Set 인덱스에 선점된 `seatId`와 만료 시각을 저장합니다.
- 선점·해제·TTL 연장 Lua Script에서 좌석 hold key와 구역 인덱스를 함께 갱신합니다.
- 좌석 조회 시 만료된 인덱스 항목을 정리한 뒤, 현재 선점된 좌석 ID만 읽습니다.
- 실제 예매와 연습 모드는 서로 다른 인덱스 key를 사용합니다.

### 재측정 결과

| 지표 | 이전 중앙값 | 인덱스 적용 후 중앙값 | 변화 |
| --- | ---: | ---: | ---: |
| hold 조회 전체 p95 | 356 ms | 175 ms | -181 ms (-50.8%) |
| Redis hold 조회 p95 | Redis `MGET` 289 ms | 인덱스 조회 175 ms | -114 ms (-39.4%) |
| 좌석 조회 p95 | 2,016 ms | 1,231 ms | -785 ms (-38.9%) |
| 전체 HTTP p95 | 1,967 ms | 1,227 ms | -740 ms (-37.6%) |

### 해석

실제 부하와 같은 100명 워밍업 뒤 3회 재측정했습니다. hold 인덱스 적용 후 Redis hold 조회 구간과
좌석 조회 p95 중앙값은 모두 감소했습니다. 다만 이전 결과는 10명 워밍업 조건으로 측정됐으므로,
전체 API 개선 폭을 hold 인덱스만의 효과로 단정하지 않습니다. 또한 재측정 실행마다 HikariCP pending 최대가
증가해, 다음 단계에서 전체 시스템 안정화 조건과 DB 커넥션 사용 경로를 추가로 확인해야 합니다.

이후 500명 이상 측정도 동일한 100명 워밍업과 HikariCP pending `0` 안정화 조건을 적용합니다.

상세 결과: [구역별 hold 인덱스 적용 후 사용자 100명](../load-test-results/2026-06-22-k6-section-hold-index-100-users/README.md)

## 다음 개선 후보

1. 예매 생성·결제 결과 처리·좌석 조회 등 DB 사용 경로별 커넥션 사용량을 분리 계측합니다.
2. 좌석 응답 직렬화와 API Gateway 구간을 세분화해 전체 좌석 조회 p95의 원인을 좁힙니다.
3. 다음 개선 효과가 확인되면 같은 조건으로 100명을 재측정하고, 결과가 안정적이면 500명과 1,000명으로 확장합니다.
