...
| 메트릭 | 측정 범위 |
| --- | --- |
| `seat_rush_response_duration_seconds{stage="mvc"}` | Ticket Service Filter부터 Controller·Interceptor·Service·Jackson JSON 직렬화가 응답 캐시에 기록될 때까지의 시간 |
| `seat_rush_response_duration_seconds{stage="copy"}` | `ContentCachingResponseWrapper`에 담긴 응답 본문을 실제 Servlet 응답으로 복사하는 시간 |
| `seat_rush_response_bytes` | Ticket Service가 만든 좌석 목록 JSON 응답 크기 |
