--[[
  ...
  ARGV:
    ...
    7: sectionIds (좌석 ID 순서와 같은 구역 ID 목록)
    8: expiresAt (만료 시각 에포크 밀리초)
    9: practiceSessionId (연습 세션 ID, 없을 경우 빈 문자열)
    10: seatCount (선점 좌석 수)
    11이후: 각 좌석의 정적 식별 ID (검증 실패 시 반환용)
--]]

-- 1. 요청된 좌석 중 이미 선점된 좌석이 있는지 확인 (EXISTS 검사)
local seatCount = tonumber(ARGV[10])
local holdKeyIndex = seatCount + 1
for index = 1, seatCount do
    if redis.call('EXISTS', KEYS[index]) == 1 then
        -- 이미 선점된 경우, 실패 코드(0)와 해당 좌석 식별 ID를 반환
        return {0, ARGV[10 + index]}
    end
end
...
for index = 1, seatCount do
    ...
    redis.call('ZADD', KEYS[holdKeyIndex + index], ARGV[8], ARGV[10 + index])
end
